(function() {
  ...
  Protocol.strdecode = function(buffer) {
    ...
    if (typeof Buffer !== "undefined" && Uint8Array === Buffer) {//后加的 解决这个问题（不知道有没有用） Maximum call stack size exceeded.
  ...
  Message.encode = function(id, type, compressRoute, route, msg) {
    ...
    // cc.logManager.info('msg.len:' + msg.length)
    if (msg) {
      msgLen += msg.length;
    }
  ...
  pomelo.init = function(params, cb) {
    ...
    var url;
    if (cc.jsInstance.remoteCfg.servers.indexOf("dtmj") >= 0) {
      url = 'wss://' + host; //正式打开 
    } else {
      url = 'ws://' + host; //测试打开
    }
  ...
  var connect = function(params, url, cb) {
    cc.logManager.info('connect to ' + url);
    ...
    var onclose = function(event) {
      ...
      // cc.logManager.info("------------ cc.jsInstance.gamenet.isConnect----pomelo--------", cc.jsInstance.gamenet.isConnect);
      // cc.logManager.info("pomelo----准备重连---", cc.director.getScene()._name);
      ...
      // } else {
      //   // cc.jsInstance.native.reConnect("pomelo");
      // }
    ...
    if (cc.sys.platform === cc.sys.WECHAT_GAME) {
      socket = wx.connectSocket({
        ...
        success: function(res) {
          cc.logManager.info("wx socket connect success");
        ...
        fail: function(res) {
          cc.logManager.error("wx socket connect fail");
        ...
        complete: function(res) {
          cc.logManager.info("wx socket connect complete");
        }
      });
  ...
  pomelo.disconnect = function() {
    if (socket) {
      //wx.closeSocket()
      // socket.close();
      cc.logManager.warn('pomelo.disconnect');
      // socket = false;

      if (socket.disconnect) socket.disconnect();
      cc.logManager.warn('pomelo-client.js, socket disconnect');
      if (socket.close) socket.close();
      cc.logManager.warn('pomelo-client.js, socket close');
      socket = null;
    }
  ...
  var processMessage = function(pomelo, msg) {
    if (!msg.id) {
      // server push message
      // cc.logManager.info(msg.route + ':');
      // cc.logManager.info(msg.body);
